    ...
    def do_clear_benke(self, content):
        ...
        if re.findall('\<tdclass=(.*?)\</tr\>', text, re.S):
            ...
            for each in list:
                ...
                sql = 'insert into college_info(院校名称,类型,所在地,提供学位,综合评价,url)' \
                      'values(\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\')' \
                      % (sc_name, type, location, provide_type, commit, url)
    ...
    def do_clear_zhuanke(self, content):
        ...
        if re.findall('\<tdclass=(.*?)\</tr\>', text, re.S):
            ...
            for each in list:
                ...
                url = 'http://www.wmzy.com' \
                      + re.findall('href=(.*?)"\>', each, re.S)[0].replace('\\"', '').replace('\\', '')
                sql = 'insert into college_info(院校名称,类型,所在地,提供学位,毕业五年月薪,url)' \
                      'values(\'%s\',\'%s\',\'%s\',\'%s\',\'%s\',\'%s\')' \
                      % (sc_name, type, location, provide_type, daiyu, url)
                # print sql
                self.cur.execute(sql)
            self.conn.commit()
