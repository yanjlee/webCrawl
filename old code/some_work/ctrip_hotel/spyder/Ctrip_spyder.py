...
class ctrip_spyder():
    ...
    def get_hotel_data(self, content, hid):
        ...
        selector = etree.HTML(content)
        hotel['hid'] = hid
        if selector.xpath('//div[@class="main_detail_wrapper "]/div/div[@class="htl_info"]/div[2]/h2/text()'):
            hotel['con'] = selector.xpath('//div[@class="main_detail_wrapper "]/div/div[@class="htl_info"]/div[2]/h2/text()')[0]
        else:
            hotel['con'] = ''
        ...
        #酒店数据采集结束,交给pipeline
        self.pipeline.deal_hotel_info(hotel)
    ...
    def get_room_data(self, content, hid):
        ...
        for each in con:
            ...
            n += 1
            #数据提交到pipeline
            self.pipeline.deal_room_info(info)
