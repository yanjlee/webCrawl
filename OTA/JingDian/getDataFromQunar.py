...
class getJingDianData(setUpSession):
    ...
    def getNum(self):
        ...
        for each in data_list:
            self.name = each.split('\u0001')[0]
            self.loc = each.split('\u0001')[1]
            self.num = each.split('\u0001')[2]
            self.type = each.split('\u0001')[3]
            self.makeSession(self.name, self.loc, self.num, self.type)
            time.sleep(3)

    def makeSession(self, name, loc, num, type):
        response = setUpSession.get_page(num)

        return self.spyder(response)
    #获取景点经纬度和电话
    ...
    def save_txt(self):
        text = str(self.jd_name) + str(self.column_split) + str(self.jd_star[0]) + str(self.column_split) +\
               ...
               str(self.jd_tips) + str(self.jd_traffic) + str(self.column_split) + \
               str(self.baidu_point) + str(self.column_split) + str(self.jd_Tel)
