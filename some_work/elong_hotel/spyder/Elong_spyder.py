...
class Elong_spyder():
    ...
    def save_hb(self, text):
        #存本地
        with open('Data/elong_hotel_base.txt', 'a', encoding='utf-8') as f:
            f.writelines(text)
        #存hdfs
        with HDFS.open("/user/cloudera/tmp/trave_hotel/elong_hotel_base.txt",'ab') as f:
            f.write(text + '\n')
    def save_hr(self, text):
        #存本地
        with open('Data/elong_room_info.txt', 'a', encoding='utf-8') as f:
            f.writelines(text)
        #存hdfs
        with HDFS.open("/user/cloudera/tmp/trave_hotel/elong_room_info.txt", 'ab') as f:
            f.write(text + '\n')
